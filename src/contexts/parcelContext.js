...
const ParcelContextProvider = (props) => {
  ...
  const getAllparcelsfromDB = async () => {
    ...
    const response = await parcelHttpService.getParcels();
  ...
  const updateParcelsInDB = async () => {
    ...
    switch (parcelExplained.action.type) {
      ...
      case ADD_PARCEL: {
        const response = await parcelHttpService.createParcel(parcelExplained.action.parcel)
        console.log('[ParcelContextProvider] updateParcelsInDB ADD_PARCEL', response);
        break;
      }
      case ADD_PARCELS: {
        const response = await parcelHttpService.addParcels(parcelExplained.action.parcels)
        console.log('[ParcelContextProvider] updateParcelsInDB ADD_PARCELS', response);
        break;
      }
      case EDIT_PARCEL: {
        const response = await parcelHttpService.updateParcel(parcelExplained.action.parcel);
        console.log('[ParcelContextProvider] updateParcelsInDB EDIT_PARCEL', response);
        break;
      }
      case REMOVE_PARCEL: {
        const response = await parcelHttpService.deletParcel(parcelExplained.action.parcelId);
