...
import { ParcelState } from '../../../redux/states/parcel/reducers';
...
const ParcelDetails = ({match, parcels} : ParcelDetailsProps) => {
    ...
    const statusFilter = { title: AppConstants.changeStatusLabel, name: "status", values: statuses, filter: setStatusFilterTerm, bullets: true, isDisabled: false };
    ...
    return currentParcel ? (
        <div className="ffh-details">
            <div className="ffh-details-header">
                ...
                <Status label={ParcelUtil.parcelStatusEnumToUIValue(currentParcel.parcelTrackingStatus)} value={currentParcel.parcelTrackingStatus} />
                ...
                <div className="ffh-toolbar__filters">
                  <Fragment key={statusFilter.title}>
                    ...
                    <Dropdown options={statuses} name={statusFilter.name} filter={statusFilter.filter} bullets={statusFilter.bullets} showOptionAll={false} isDisabled={statusFilter.isDisabled}/>
                  </Fragment>
                </div>
            </div>
...
export default withRouter(connect(mapStateToProps)(ParcelDetails));
