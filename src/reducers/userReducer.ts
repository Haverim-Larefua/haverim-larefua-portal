import { ADD_USER, ADD_USERS, EDIT_USER, REMOVE_USER, LOAD_USERS, SEARCH_USERS, UPDATE_USERS_AREAS, 
    IActionBase, addUser, addUsers, editUser, loadUsers, searchUsers, removeUser } from "../contexts/actions/users.action";
...
export const userReducer = ( state: UserExtendedData = defaultUserExtendedData, action: IActionBase) => {
  ...
  switch (action.type) {
    ...
    case SEARCH_USERS: {
      ...
      return { users: state.users, action: loadUsers([]), searchParams: action.searchParams, deliveryAreas: state.deliveryAreas};
    }
    case ADD_USER: {
    ...
    case UPDATE_USERS_AREAS : {
      return { users: state.users, action: action, searchParams: state.searchParams, deliveryAreas: UserUtil.getUsersAreasDistinct(state.users) };
    }
    default:
      return state;
  }
